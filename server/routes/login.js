...
app.post('/google', async(req, res) => {
    ...
    //googleUser tendra la inf del usuario
    Usuario.findOne({ email: googleUser.email }, (err, usuarioDB) => {
        ...
        if (usuarioDB) {
            if (usuarioDB.google === false) { //El usuario ya tiene una cuenta normal sin google, por ello debe ingresar con ella
                return res.status(400).json({
                    ok: false,
                    err: {
            ...
            } else { //si el usuario si es de google, entonces se renueva su token y se retorna
                let token = jwt.sign({
                    usuario: usuarioDB
                }, process.env.SEED, { expiresIn: process.env.CADUCIDAD_TOKEN });
